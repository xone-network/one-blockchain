...
pip install j2cli
CLI_DEB_BASE="one-blockchain-cli_$ONE_INSTALLER_VERSION-1_$PLATFORM"
mkdir -p "dist/$CLI_DEB_BASE/opt/one"
mkdir -p "dist/$CLI_DEB_BASE/usr/bin"
mkdir -p "dist/$CLI_DEB_BASE/DEBIAN"
j2 -o "dist/$CLI_DEB_BASE/DEBIAN/control" assets/deb/control.j2
cp -r dist/daemon/* "dist/$CLI_DEB_BASE/opt/one/"
ln -s ../../opt/one/one "dist/$CLI_DEB_BASE/usr/bin/one"
...
if [ "$PLATFORM" = "arm64" ]; then
  ...
  echo USE_SYSTEM_FPM=true electron-builder build --linux deb --arm64 \
    ...
    --config.deb.packageName="one-blockchain"
...
GUI_DEB_NAME=one-blockchain_${ONE_INSTALLER_VERSION}_${PLATFORM}.deb
mv "dist/${PRODUCT_NAME}-${ONE_INSTALLER_VERSION}.deb" "../../../build_scripts/dist/${GUI_DEB_NAME}"
