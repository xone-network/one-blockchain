...
if [ ! "$ONE_INSTALLER_VERSION" ]; then
	echo "WARNING: No environment variable ONE_INSTALLER_VERSION set. Using 0.0.0."
	ONE_INSTALLER_VERSION="0.0.0"
fi
echo "One Installer Version is: $ONE_INSTALLER_VERSION"
...
SPEC_FILE=$(python -c 'import one; print(one.PYINSTALLER_SPEC_PATH)')
...
# Builds CLI only rpm
CLI_RPM_BASE="one-blockchain-cli-$ONE_INSTALLER_VERSION-1.$REDHAT_PLATFORM"
mkdir -p "dist/$CLI_RPM_BASE/opt/one"
mkdir -p "dist/$CLI_RPM_BASE/usr/bin"
cp -r dist/daemon/* "dist/$CLI_RPM_BASE/opt/one/"
ln -s ../../opt/one/one "dist/$CLI_RPM_BASE/usr/bin/one"
...
fpm -s dir -t rpm \
  ...
  --name one-blockchain-cli \
  --license Apache-2.0 \
  --version "$ONE_INSTALLER_VERSION" \
  --architecture "$REDHAT_PLATFORM" \
  --description "One is a modern cryptocurrency built from scratch, designed to be efficient, decentralized, and secure." \
  --depends /usr/lib64/libcrypt.so.1 \
  .
# CLI only rpm done

cp -r dist/daemon ../one-blockchain-gui/packages/gui

# Change to the gui package
cd ../one-blockchain-gui/packages/gui || exit 1

# sets the version for one-blockchain in package.json
cp package.json package.json.orig
jq --arg VER "$ONE_INSTALLER_VERSION" '.version=$VER' package.json > temp.json && mv temp.json package.json
...
GUI_RPM_NAME="one-blockchain-${ONE_INSTALLER_VERSION}-1.${REDHAT_PLATFORM}.rpm"
mv "dist/${PRODUCT_NAME}-${ONE_INSTALLER_VERSION}.rpm" "../../../build_scripts/dist/${GUI_RPM_NAME}"
