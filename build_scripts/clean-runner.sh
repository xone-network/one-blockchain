...
rm -rf ../venv || true
rm -rf venv || true
rm -rf one_blockchain.egg-info || true
rm -rf build_scripts/final_installer || true
rm -rf build_scripts/dist || true
rm -rf build_scripts/pyinstaller || true
rm -rf one-blockchain-gui/build || true
rm -rf one-blockchain-gui/daemon || true
rm -rf one-blockchain-gui/node_modules || true
rm one-blockchain-gui/temp.json || true
( cd "$PWD/one-blockchain-gui" && git checkout HEAD -- package-lock.json ) || true
cd "$PWD" || true
