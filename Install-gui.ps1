...
{
    Write-Output "This script requires that the One Python virtual environment is activated."
...
Push-Location
try {
    Set-Location one-blockchain-gui
    ...
    Write-Output ""
    Write-Output "One blockchain Install-gui.ps1 completed."
    Write-Output ""
    Write-Output "Type 'cd one-blockchain-gui' and then 'npm run electron' to start the GUI."
} finally {
    Pop-Location
}
