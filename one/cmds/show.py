...
from one.cmds.show_funcs import show_async
...
def show_cmd(
    ...
    if connections:
        print("'one show -c' has been renamed to 'one peer -c' ")
    if add_connection != "":
        print("'one show -a' has been renamed to 'one peer -a' ")
    if remove_connection != "":
        print("'one show -r' has been renamed to 'one peer -r' ")
    if wallet_rpc_port is not None:
        print("'one show -wp' is not used, please remove it from your command.")
    asyncio.run(
        show_async(
            rpc_port,
            ctx.obj["root_path"],
