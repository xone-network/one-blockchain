...
async def summary(
    ...
    if amounts is not None:
        print(f"Total one farmed: {amounts['farmed_amount'] / units['one']}")
        print(f"User transaction fees: {amounts['fee_amount'] / units['one']}")
        print(f"Block rewards: {(amounts['farmer_reward_amount'] + amounts['pool_reward_amount']) / units['one']}")
    ...
    if amounts is None:
        if wallet_not_running:
            print("For details on farmed rewards and fees you should run 'one start wallet' and 'one wallet show'")
        elif wallet_not_ready:
            print("For details on farmed rewards and fees you should run 'one wallet show'")
    else:
        print("Note: log into your key using 'one wallet show' to see rewards for each key")
