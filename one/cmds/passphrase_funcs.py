...
from one.daemon.client import acquire_connection_to_daemon
from one.util.config import load_config
from one.util.errors import KeychainMaxUnlockAttempts
from one.util.keychain import Keychain, supports_os_passphrase_storage
from one.util.keyring_wrapper import KeyringWrapper, DEFAULT_PASSPHRASE_IF_NO_MASTER_PASSPHRASE
from one.util.misc import prompt_yes_no
from one.util.ws_message import WsRpcMessage
    ...
    try:
        if supports_os_passphrase_storage():
            ...
            print(
                ...
                "Other One processes will be able to access your keys without prompting for your passphrase."
            )
...
def read_passphrase_from_file(passphrase_file: TextIOWrapper) -> str:
    passphrase = passphrase_file.read().rstrip(os.environ.get("ONE_PASSPHRASE_STRIP_TRAILING_CHARS", "\r\n"))
...
def initialize_passphrase() -> None:
    if Keychain.has_master_passphrase():
        ...
        print("\nUse 'one passphrase set' or 'one passphrase remove' to update or remove your passphrase")
        sys.exit(1)
