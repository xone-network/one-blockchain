...
from one.cmds.cmds_util import transaction_status_msg, transaction_submitted_msg
from one.cmds.peer_funcs import print_connections
from one.cmds.units import units
from one.rpc.wallet_rpc_client import WalletRpcClient
from one.server.start_wallet import SERVICE_NAME
from one.types.blockchain_format.sized_bytes import bytes32
from one.util.bech32m import bech32_decode, decode_puzzle_hash, encode_puzzle_hash
from one.util.config import load_config, selected_network_address_prefix
from one.util.default_root import DEFAULT_ROOT_PATH
from one.util.ints import uint16, uint32, uint64
from one.wallet.nft_wallet.nft_info import NFTInfo
...
from one.wallet.trading.trade_status import TradeStatus
from one.wallet.transaction_record import TransactionRecord
from one.wallet.util.address_type import AddressType, ensure_valid_address
...
def print_transaction(tx: TransactionRecord, verbose: bool, name, address_prefix: str, mojo_per_unit: int) -> None:
    if verbose:
        ...
        one_amount = Decimal(int(tx.amount)) / mojo_per_unit
        ...
        print(f"Amount {description}: {one_amount} {name}")
...
async def send(args: dict, wallet_client: WalletRpcClient, fingerprint: int) -> None:
    ...
    final_fee = uint64(int(fee * units["one"]))
    final_amount: uint64
    final_min_coin_amount: uint64
    if typ == WalletType.STANDARD_WALLET:
        final_amount = uint64(int(amount * units["one"]))
        final_min_coin_amount = uint64(int(min_coin_amount * units["one"]))
    ...
    print(f"To get status, use command: one wallet get_transaction -f {fingerprint} -tx 0x{tx_id}")
        ...
        for item in [*offers, *requests]:
            ...
            if confirmation not in ["y", "yes"]:
                ...
                if offer is not None:
                    ...
                    print(f"Use one wallet get_offers --id {trade_record.trade_id} -f {fingerprint} to view status")
                else:
...
async def print_trade_record(record, wallet_client: WalletRpcClient, summaries: bool = False) -> None:
    ...
    if summaries:
        ...
        print(f"Included Fees: {fees / units['one']}")
    print("---------------")
...
async def take_offer(args: dict, wallet_client: WalletRpcClient, fingerprint: int) -> None:
    ...
    if royalty_asset_dict != {}:
        ...
        for fungible_asset_id in fungible_assets_from_offer(offer):
            fungible_asset_id_str = fungible_asset_id.hex() if fungible_asset_id is not None else "xone"
    ...
    print(f"Included Fees: {Decimal(offer.bundle.fees()) / units['one']}")
    ...
    if not examine_only:
        ...
        if confirmation in ["y", "yes"]:
            ...
            print(f"Use one wallet get_offers --id {trade_record.trade_id} -f {fingerprint} to view its status")
...
async def cancel_offer(args: dict, wallet_client: WalletRpcClient, fingerprint: int) -> None:
    ...
    if confirmation in ["y", "yes"]:
        ...
        if secure:
            print(f"Use one wallet get_offers --id {trade_record.trade_id} -f {fingerprint} to view cancel status")
...
def wallet_coin_unit(typ: WalletType, address_prefix: str) -> Tuple[str, int]:
    ...
    if typ in [WalletType.STANDARD_WALLET, WalletType.POOLING_WALLET, WalletType.MULTI_SIG]:
        return address_prefix, units["one"]
    return "", units["mojo"]
...
async def send_notification(args: Dict, wallet_client: WalletRpcClient, fingerprint: int) -> None:
    ...
    amount: uint64 = uint64(Decimal(args["amount"]) * units["one"])
    message: bytes = bytes(args["message"], "utf8")
    fee: uint64 = uint64(Decimal(args["fee"]) * units["one"])

    tx = await wallet_client.send_notification(address, message, amount, fee)

    print("Notification sent successfully.")
    print(f"To get status, use command: one wallet get_transaction -f {fingerprint} -tx 0x{tx.name}")
