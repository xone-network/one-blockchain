...
@click.option("--testnet", is_flag=True, help="Configure this one install to connect to the testnet")
...
def init_cmd(ctx: click.Context, create_certs: str, fix_ssl_permissions: bool, testnet: bool, v1_db: bool, **kwargs):
    ...
    - Make a copy of your Farming Machine CA directory: ~/.one/[version]/config/ssl/ca
    - Shut down all one daemon processes with `one stop all -d`
    - Run `one init -c [directory]` on your remote harvester,
      where [directory] is the the copy of your Farming Machine CA directory
    - Get more details on remote harvester on One wiki:
      https://github.com/xone-network/one-blockchain/wiki/Farming-on-many-machines
    """
    from pathlib import Path

    from one.cmds.passphrase_funcs import initialize_passphrase
...
if __name__ == "__main__":
    ...
    from .init_funcs import one_init

    one_init(DEFAULT_ROOT_PATH)
