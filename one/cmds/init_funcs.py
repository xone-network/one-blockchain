...
from one.ssl.create_ssl import (
    ensure_ssl_dirs,
...
from one.util.config import (
    create_default_one_config,
    initial_config_file,
    load_config,
...
from one.cmds.configure import configure
...
def check_keys(new_root: Path, keychain: Optional[Keychain] = None) -> None:
    ...
    if len(all_sks) == 0:
        print("No keys are present in the keychain. Generate them with 'one keys generate'")
        return None

    with lock_and_load_config(new_root, "config.yaml") as config:
        pool_child_pubkeys = [master_sk_to_pool_sk(sk).get_g1() for sk, _ in all_sks]
        all_targets = []
        stop_searching_for_farmer = "xone_target_address" not in config["farmer"]
        stop_searching_for_pool = "xone_target_address" not in config["pool"]
        ...
        for i in range(number_of_ph_to_search):
            ...
            for sk, _ in all_sks:
                ...
                if all_targets[-1] == config["farmer"].get("xone_target_address") or all_targets[-2] == config[
                    "farmer"
                ].get("xone_target_address"):
                    stop_searching_for_farmer = True
                if all_targets[-1] == config["pool"].get("xone_target_address") or all_targets[-2] == config["pool"].get(
                    "xone_target_address"
                ):
        ...
        if "xone_target_address" not in config["farmer"]:
            print(
                f"Setting the xone destination for the farmer reward (1/8 plus fees, solo and pooling)"
                f" to {all_targets[0]}"
            )
            config["farmer"]["xone_target_address"] = all_targets[0]
            updated_target = True
        elif config["farmer"]["xone_target_address"] not in all_targets:
            print(
                f"WARNING: using a farmer address which we might not have the private"
                f" keys for. We searched the first {number_of_ph_to_search} addresses. Consider overriding "
                f"{config['farmer']['xone_target_address']} with {all_targets[0]}"
            )

        if "pool" not in config:
            config["pool"] = {}
        if "xone_target_address" not in config["pool"]:
            print(f"Setting the xone destination address for pool reward (7/8 for solo only) to {all_targets[0]}")
            config["pool"]["xone_target_address"] = all_targets[0]
            updated_target = True
        elif config["pool"]["xone_target_address"] not in all_targets:
            print(
                f"WARNING: using a pool address which we might not have the private"
                f" keys for. We searched the first {number_of_ph_to_search} addresses. Consider overriding "
                f"{config['pool']['xone_target_address']} with {all_targets[0]}"
            )
        if "xone_target_address" not in config["timelord"]:
            print(
                f"Setting the chiax destination for the timelord fee (1/100 plus fees)"
                f" to {all_targets[0]}"
            )
            config["timelord"]["xone_target_address"] = all_targets[0]
            updated_target = True
        elif config["timelord"]["xone_target_address"] not in all_targets:
            print(
                f"WARNING: using a farmer address which we might not have the private"
                f" keys for. We searched the first {number_of_ph_to_search} addresses. Consider overriding "
...
def create_all_ssl(
    ...
    *,
    ...
    one_ca_crt, one_ca_key = get_one_ca_crt_key()
    one_ca_crt_path = ca_dir / "one_ca.crt"
    one_ca_key_path = ca_dir / "one_ca.key"
    write_ssl_cert_and_key(one_ca_crt_path, one_ca_crt, one_ca_key_path, one_ca_key, overwrite=overwrite)
...
def init(
    ...
    if create_certs is not None:
        ...
        return one_init(root_path, fix_ssl_permissions=fix_ssl_permissions, testnet=testnet, v1_db=v1_db)


def one_version_number() -> Tuple[str, str, str, str]:
...
def one_full_version_str() -> str:
    major, minor, patch, dev = one_version_number()
...
def one_init(
    ...
    *,
    ...
    """
    one_root = os.environ.get("ONE_ROOT", None)
    if one_root is not None:
        print(f"ONE_ROOT is set to {one_root}")

    print(f"One directory {root_path}")
    if root_path.is_dir() and Path(root_path / "config" / "config.yaml").exists():
        # This is reached if ONE_ROOT is set, or if user has run one init twice
        # before a new update.
        if testnet:
            configure(
                root_path,
    ...
    print("")
    print("To see your keys, run 'one keys show --show-mnemonic-seed'")
