...
from one.cmds.cmds_util import get_any_service_client, transaction_status_msg, transaction_submitted_msg
from one.cmds.units import units
from one.cmds.wallet_funcs import print_balance, wallet_coin_unit
from one.pools.pool_config import PoolWalletConfig, load_pool_config, update_pool_config
from one.pools.pool_wallet_info import PoolSingletonState, PoolWalletInfo
...
async def create(args: dict, wallet_client: WalletRpcClient, fingerprint: int) -> None:
    ...
    fee_mojos = uint64(int(fee * units["one"]))
    ...
    if user_input.lower() == "y" or user_input.lower() == "yes":
        try:
            ...
            print(f"Error creating plot NFT: {e}\n    Please start both farmer and wallet with:  one start -r farmer")
        return
    print("Aborting.")
