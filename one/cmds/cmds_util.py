...
from one.daemon.keychain_proxy import KeychainProxy, connect_to_keychain_and_validate
from one.rpc.data_layer_rpc_client import DataLayerRpcClient
from one.rpc.farmer_rpc_client import FarmerRpcClient
from one.rpc.full_node_rpc_client import FullNodeRpcClient
from one.rpc.harvester_rpc_client import HarvesterRpcClient
...
from one.types.mempool_submission_status import MempoolSubmissionStatus
...
from one.util.keychain import KeyData
...
def transaction_status_msg(fingerprint: int, tx_id: bytes32) -> str:
    return f"Run 'one wallet get_transaction -f {fingerprint} -tx 0x{tx_id}' to get status"
...
async def get_wallet(root_path: Path, wallet_client: WalletRpcClient, fingerprint: Optional[int]) -> Optional[int]:
    ...
    if fingerprint is not None:
        ...
        if len(fingerprints) == 0:
            print("No keys loaded. Run 'one keys generate' or import a key")
