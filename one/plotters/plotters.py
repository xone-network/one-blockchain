...
from one.plotters.bladebit import get_bladebit_install_info, plot_bladebit
from one.plotters.chiapos import get_chiapos_install_info, plot_one
from one.plotters.madmax import get_madmax_install_info, plot_madmax
...
def call_plotters(root_path: Path, args):
    # Add `plotters` section in ONE_ROOT.
    one_root_path = root_path
    ...
    if not os.path.exists(root_path):
        print(f"Creating plotters folder within ONE_ROOT: {root_path}")
        try:
            os.mkdir(root_path)
        except Exception as e:
            print(f"Cannot create plotters root path {root_path} {type(e)} {e}.")

    plotters = argparse.ArgumentParser("one plotters", description="Available options.")
    subparsers = plotters.add_subparsers(help="Available options", dest="plotter")

    build_parser(subparsers, root_path, chia_plotter_options, "chiapos", "Create a plot with the default one plotter")
    ...
    deprecation_warning = (
        "[DEPRECATED] 'one plotters install' is no longer available. Use install-plotter.sh/ps1 instead."
    )
    ...
    deprecation_warning_bb2 = "[DEPRECATED] 'one plotters bladebit2' was integrated to 'one plotters bladebit'"
    ...
    if args.plotter is None:
        ...
        plot_one(args, one_root_path)
    elif args.plotter == "madmax":
        plot_madmax(args, one_root_path, root_path)
    elif args.plotter == "bladebit":
        plot_bladebit(args, one_root_path, root_path)
    elif args.plotter == "version":
        show_plotters_version(one_root_path)
