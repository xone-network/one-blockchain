...
from one.data_layer.data_layer_errors import OfferIntegrityError
from one.data_layer.data_layer_util import OfferStore, ProofOfInclusion, ProofOfInclusionLayer, StoreProofs, leaf_hash
...
from one.wallet.db_wallet.db_wallet_puzzles import (
    ACS_MU,
    ACS_MU_PH,
    GRAFTROOT_DL_OFFERS,
    SINGLETON_LAUNCHER,
...
from one.wallet.puzzles.singleton_top_layer_v1_1 import SINGLETON_LAUNCHER_HASH
from one.wallet.sign_coin_spends import sign_coin_spends
from one.wallet.trading.offer import NotarizedPayment, Offer
from one.wallet.transaction_record import TransactionRecord
from one.wallet.util.compute_memos import compute_memos
from one.wallet.util.merkle_utils import _simplify_merkle_proof
...
class DataLayerWallet:
    ...
    async def get_launcher_coin_state(self, launcher_id: bytes32, peer: WSOneConnection) -> CoinState:
    ...
    async def delete_mirror(
        self, mirror_id: bytes32, peer: WSOneConnection, fee: uint64 = uint64(0)
    ) -> List[TransactionRecord]:
        ...
        if excess_fee > 0:
            one_tx: TransactionRecord = await self.wallet_state_manager.main_wallet.generate_signed_transaction(
                uint64(1),
                new_puzhash,
            ...
            txs = [
                dataclasses.replace(
                    txs[0], spend_bundle=SpendBundle.aggregate([txs[0].spend_bundle, one_tx.spend_bundle])
                ),
                dataclasses.replace(one_tx, spend_bundle=None),
            ]
