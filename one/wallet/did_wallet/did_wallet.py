...
from one.wallet.coin_selection import select_coins
from one.wallet.derivation_record import DerivationRecord
from one.wallet.derive_keys import master_sk_to_wallet_sk_unhardened
from one.wallet.did_wallet import did_wallet_puzzles
from one.wallet.did_wallet.did_info import DIDInfo
from one.wallet.did_wallet.did_wallet_puzzles import create_fullpuz, uncurry_innerpuz
...
class DIDWallet:
    ...
    async def coin_added(self, coin: Coin, _: uint32, peer: WSOneConnection):
    ...
    async def load_parent(self, did_info: DIDInfo):
        ...
        peer: WSOneConnection = wallet_node.get_full_node_peer()
        if peer is None:
    ...
    async def create_update_spend(self, fee: uint64 = uint64(0)):
        ...
        if fee > 0:
            ...
            one_tx = await self.create_tandem_xone_tx(fee, Announcement(coin.name(), announcement_to_make))
        else:
        ...
        if one_tx is not None and one_tx.spend_bundle is not None:
            ...
            await self.wallet_state_manager.add_pending_transaction(one_tx)
