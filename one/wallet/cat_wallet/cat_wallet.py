...
from one.wallet.cat_wallet.cat_info import CATInfo, LegacyCATInfo
...
from one.wallet.puzzles.tails import ALL_LIMITATIONS_PROGRAMS
...
class CATWallet:
    ...
    async def create_new_cat_wallet(
        ...
        try:
            one_tx, spend_bundle = await ALL_LIMITATIONS_PROGRAMS[
        ...
        one_tx = dataclasses.replace(one_tx, spend_bundle=spend_bundle)
        await self.standard_wallet.push_transaction(one_tx)
    ...
    async def create_tandem_xone_tx(
        ...
        if fee > amount_to_claim:
            one_coins = await self.standard_wallet.select_coins(fee, min_coin_amount=min_coin_amount)
            origin_id = list(one_coins)[0].name()
            ...
            message = None
            for spend in one_tx.spend_bundle.coin_spends:
            ...
            one_coins = await self.standard_wallet.select_coins(fee, min_coin_amount=min_coin_amount)
            selected_amount = sum([c.amount for c in one_coins])
            one_tx = await self.standard_wallet.generate_signed_transaction(
    ...
    async def generate_unsigned_spendbundle(
        ...
        # Figure out if we need to absorb/melt some XONE as part of this
        regular_one_to_claim: int = 0
        if payment_amount > starting_amount:
            fee = uint64(fee + payment_amount - starting_amount)
        elif payment_amount < starting_amount:
            regular_one_to_claim = payment_amount

        need_one_transaction = (fee > 0 or regular_one_to_claim > 0) and (fee - regular_one_to_claim != 0)
        ...
        for coin in cat_coins:
            if first:
                ...
                if need_one_transaction:
                    if fee > regular_one_to_claim:
                        one_tx, _ = await self.create_tandem_xone_tx(
                            fee,
                            uint64(regular_one_to_claim),
                    ...
                    elif regular_one_to_claim > fee:
                        one_tx, _ = await self.create_tandem_xone_tx(
                            fee, uint64(regular_one_to_claim), min_coin_amount=min_coin_amount
                        )
                        innersol = self.standard_wallet.make_solution(
                            primaries=primaries,
                            coin_announcements={announcement.message},
        ...
        one_spend_bundle = SpendBundle([], G2Element())
        if one_tx is not None and one_tx.spend_bundle is not None:
            one_spend_bundle = one_tx.spend_bundle
    ...
    async def generate_signed_transaction(
        ...
        if one_tx is not None:
            tx_list.append(
                TransactionRecord(
                    confirmed_at_height=one_tx.confirmed_at_height,
                    created_at_time=one_tx.created_at_time,
                    to_puzzle_hash=one_tx.to_puzzle_hash,
                    amount=one_tx.amount,
                    fee_amount=one_tx.fee_amount,
                    confirmed=one_tx.confirmed,
                    sent=one_tx.sent,
                    spend_bundle=None,
                    additions=one_tx.additions,
                    removals=one_tx.removals,
                    wallet_id=one_tx.wallet_id,
                    sent_to=one_tx.sent_to,
                    trade_id=one_tx.trade_id,
                    type=one_tx.type,
                    name=one_tx.name,
                    memos=[],
                )
