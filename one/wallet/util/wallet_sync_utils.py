...
from one.util.merkle_set import MerkleSet, confirm_included_already_hashed, confirm_not_included_already_hashed
from one.wallet.util.peer_request_cache import PeerRequestCache
...
async def fetch_last_tx_from_peer(height: uint32, peer: WSOneConnection) -> Optional[HeaderBlock]:
...
async def request_and_validate_removals(
    peer: WSOneConnection, height: uint32, header_hash: bytes32, coin_name: bytes32, removals_root: bytes32
) -> bool:
...
async def request_header_blocks(
    peer: WSOneConnection, start_height: uint32, end_height: uint32
...
async def _fetch_header_blocks_inner(
    all_peers: List[Tuple[WSOneConnection, bool]],
