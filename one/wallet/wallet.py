...
from one.wallet.puzzles.puzzle_utils import (
...
from one.wallet.secret_key_store import SecretKeyStore
...
class Wallet:
    ...
    async def create_spend_bundle_relative_one(self, one_amount: int, exclude: List[Coin] = []) -> SpendBundle:
        list_of_solutions = []
        utxos = None
        ...
        if one_amount < 0:
            utxos = await self.select_coins(uint64(abs(one_amount)), exclude)
        else:
        ...
        one_amount = spend_value + one_amount
        ...
        for coin in utxos:
            ...
            if output_created is None:
                ...
                primaries: List[AmountWithPuzzlehash] = [
                    {"puzzlehash": newpuzhash, "amount": uint64(one_amount), "memos": []}
                ]
