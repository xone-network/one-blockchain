...
from one.data_layer.dl_wallet_store import DataLayerStore
from one.pools.pool_puzzles import SINGLETON_LAUNCHER_HASH, solution_to_pool_state
from one.pools.pool_wallet import PoolWallet
from one.protocols import wallet_protocol
...
from one.wallet.cat_wallet.cat_utils import construct_cat_puzzle, match_cat_puzzle
from one.wallet.cat_wallet.cat_wallet import CATWallet
from one.wallet.db_wallet.db_wallet_puzzles import MIRROR_PUZZLE_HASH
...
from one.wallet.did_wallet.did_wallet_puzzles import DID_INNERPUZ_MOD, create_fullpuz, match_did_puzzle
from one.wallet.key_val_store import KeyValStore
from one.wallet.nft_wallet.nft_info import NFTWalletInfo
from one.wallet.nft_wallet.nft_puzzles import get_metadata_and_phs, get_new_owner_did
from one.wallet.nft_wallet.nft_wallet import NFTWallet
from one.wallet.nft_wallet.uncurry_nft import UncurriedNFT
from one.wallet.notification_manager import NotificationManager
from one.wallet.outer_puzzles import AssetType
from one.wallet.puzzle_drivers import PuzzleInfo
from one.wallet.puzzles.cat_loader import CAT_MOD, CAT_MOD_HASH
from one.wallet.settings.user_settings import UserSettings
from one.wallet.trade_manager import TradeManager
from one.wallet.transaction_record import TransactionRecord
from one.wallet.uncurried_puzzle import uncurry_puzzle
from one.wallet.util.address_type import AddressType
from one.wallet.util.compute_hints import compute_coin_hints
from one.wallet.util.transaction_type import TransactionType
from one.wallet.util.wallet_sync_utils import PeerRequestException, last_change_height_cs
from one.wallet.util.wallet_types import WalletType
from one.wallet.wallet import Wallet
from one.wallet.wallet_blockchain import WalletBlockchain
from one.wallet.wallet_coin_record import WalletCoinRecord
from one.wallet.wallet_coin_store import WalletCoinStore
from one.wallet.wallet_info import WalletInfo
from one.wallet.wallet_interested_store import WalletInterestedStore
from one.wallet.wallet_nft_store import WalletNftStore
from one.wallet.wallet_pool_store import WalletPoolStore
from one.wallet.wallet_protocol import WalletProtocol
from one.wallet.wallet_puzzle_store import WalletPuzzleStore
from one.wallet.wallet_retry_store import WalletRetryStore
from one.wallet.wallet_transaction_store import WalletTransactionStore
from one.wallet.wallet_user_store import WalletUserStore
...
class WalletStateManager:
    ...
    async def determine_coin_type(
        self, peer: WSOneConnection, coin_state: CoinState, fork_height: Optional[uint32]
    ...
    async def filter_spam(self, new_coin_state: List[CoinState]) -> List[CoinState]:
        xone_spam_amount = self.config.get("xone_spam_amount", 1000000)

        # No need to filter anything if the filter is set to 1 or 0 mojos
        if xone_spam_amount <= 1:
            return new_coin_state

        spam_filter_after_n_txs = self.config.get("spam_filter_after_n_txs", 200)
        small_unspent_count = await self.coin_store.count_small_unspent(xone_spam_amount)
        ...
        for cs in new_coin_state:
            ...
            if (
                ...
                and cs.coin.amount < xone_spam_amount
    ...
    async def get_minter_did(self, launcher_coin: Coin, peer: WSOneConnection) -> Optional[bytes32]:
        # Get minter DID
