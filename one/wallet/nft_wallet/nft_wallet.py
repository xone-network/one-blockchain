...
from one.protocols.wallet_protocol import CoinState
from one.server.ws_connection import WSOneConnection
from one.types.announcement import Announcement
...
from one.util.ints import uint8, uint16, uint32, uint64, uint128
from one.wallet.derivation_record import DerivationRecord
from one.wallet.did_wallet import did_wallet_puzzles
from one.wallet.did_wallet.did_info import DIDInfo
from one.wallet.lineage_proof import LineageProof
from one.wallet.nft_wallet import nft_puzzles
from one.wallet.nft_wallet.nft_info import NFTCoinInfo, NFTWalletInfo
from one.wallet.nft_wallet.nft_puzzles import NFT_METADATA_UPDATER, create_ownership_layer_puzzle, get_metadata_and_phs
from one.wallet.nft_wallet.uncurry_nft import UncurriedNFT
from one.wallet.outer_puzzles import AssetType, construct_puzzle, match_puzzle, solve_puzzle
from one.wallet.payment import Payment
from one.wallet.puzzle_drivers import PuzzleInfo, Solver
from one.wallet.puzzles.p2_delegated_puzzle_or_hidden_puzzle import (
    DEFAULT_HIDDEN_PUZZLE_HASH,
    calculate_synthetic_secret_key,
    puzzle_for_pk,
)
from one.wallet.trading.offer import (
    OFFER_MOD,
    OFFER_MOD_HASH,
    OFFER_MOD_OLD,
    OFFER_MOD_OLD_HASH,
...
from one.wallet.uncurried_puzzle import uncurry_puzzle
from one.wallet.util.compute_memos import compute_memos
from one.wallet.util.debug_spend_bundle import disassemble
from one.wallet.util.transaction_type import TransactionType
from one.wallet.util.wallet_types import AmountWithPuzzlehash, WalletType
from one.wallet.wallet import CHIP_0002_SIGN_MESSAGE_PREFIX, Wallet
from one.wallet.wallet_coin_record import WalletCoinRecord
from one.wallet.wallet_info import WalletInfo
from one.wallet.wallet_nft_store import WalletNftStore
...
class NFTWallet:
    ...
    async def coin_added(self, coin: Coin, height: uint32, peer: WSOneConnection) -> None:
    ...
    async def puzzle_solution_received(self, coin_spend: CoinSpend, peer: WSOneConnection) -> None:
    ...
    async def create_tandem_xone_tx(
        self, fee: uint64, announcement_to_assert: Optional[Announcement] = None
    ) -> TransactionRecord:
        one_coins = await self.standard_wallet.select_coins(fee)
        one_tx = await self.standard_wallet.generate_signed_transaction(
            uint64(0),
        ...
        assert one_tx.spend_bundle is not None
        return one_tx
    ...
    async def generate_signed_transaction(
        ...
        unsigned_spend_bundle, one_tx = await self.generate_unsigned_spendbundle(
            payments,
            fee,
            coins=coins,
            nft_coin=nft_coin,
        ...
        if one_tx is not None and one_tx.spend_bundle is not None:
            spend_bundle = SpendBundle.aggregate([spend_bundle, one_tx.spend_bundle])
            one_tx = dataclasses.replace(one_tx, spend_bundle=None)
        ...
        if one_tx is not None:
            tx_list.append(one_tx)
    ...
    async def generate_unsigned_spendbundle(
        ...
        if fee > 0:
            ...
            one_tx = await self.create_tandem_xone_tx(fee, Announcement(nft_coin.coin.name(), announcement_to_make))
        else:
        ...
        return nft_spend_bundle, one_tx
    ...
    async def mint_from_did(
        ...
        xone_coins: Optional[Set[Coin]] = None,
        xone_change_ph: Optional[bytes32] = None,
        ...
        - The launcher coins are then spent along with the created eve spend
          and an xone spend that funds the transactions and pays fees.
        ...
        :param xone_coins: [Optional] For use with bulk minting to provide the coin used for funding the minting spend.
        This coin can be one that will be created in the future
        :param xone_change_ph: [Optional] For use with bulk minting, so we can specify the puzzle hash that the change
        ...
        :param fee: A fee amount, taken out of the xone spend.
        """
        # get DID Wallet
        ...
        # Ensure we have an xone coin of high enough amount
        assert isinstance(fee, uint64)
        total_amount = len(metadata_list) + fee
        if xone_coins is None:
            xone_coins = await self.standard_wallet.select_coins(uint64(total_amount))
        assert len(xone_coins) > 0
        ...
        # Create the xone spend to fund the minting.
        spend_value = sum([coin.amount for coin in xone_coins])
        change: uint64 = uint64(spend_value - total_amount)
        xone_spends = []
        if xone_change_ph is None:
            xone_change_ph = await self.standard_wallet.get_new_puzzlehash()
        xone_primaries = [
            AmountWithPuzzlehash({"puzzlehash": xone_change_ph, "amount": change, "memos": [xone_change_ph]})
        ]

        first = True
        for xone_coin in xone_coins:
            puzzle: Program = await self.standard_wallet.puzzle_for_puzzle_hash(xone_coin.puzzle_hash)
            if first:
                message_list: List[bytes32] = [c.name() for c in xone_coins]
                message_list.append(
                    Coin(xone_coin.name(), xone_primaries[0]["puzzlehash"], xone_primaries[0]["amount"]).name()
                )
                message: bytes32 = std_hash(b"".join(message_list))

                if len(xone_coins) > 1:
                    xone_announcement: Optional[Set[bytes]] = {message}
                else:
                    xone_announcement = None

                solution: Program = self.standard_wallet.make_solution(
                    primaries=xone_primaries,
                    fee=fee,
                    coin_announcements=xone_announcement,
                    coin_announcements_to_assert={Announcement(did_coin.name(), message).name()},
                )
                primary_announcement_hash = Announcement(xone_coin.name(), message).name()
            ...
            xone_spends.append(CoinSpend(xone_coin, puzzle, solution))
        xone_spend = await self.standard_wallet.sign_transaction(xone_spends)
        ...
        total_spend = SpendBundle.aggregate([signed_spend_bundle, xone_spend, *eve_spends])
        return total_spend

    async def mint_from_xone(
        self,
        metadata_list: List[Dict[str, Any]],
        target_list: Optional[List[bytes32]] = [],
        mint_number_start: Optional[int] = 1,
        ...
        funding_coin = xone_coins.copy().pop()
        ...
        for mint_number in range(mint_number_start, mint_number_end):
            ...
            # Annnouncements for eve spend. These are asserted by the xone spend
            announcement_message = Program.to([eve_fullpuz.get_tree_hash(), amount, []]).get_tree_hash()
        ...
        for xone_coin in xone_coins:
            ...
            if first:
                ...
                solution: Program = self.standard_wallet.make_solution(
                    primaries=xone_primaries + primaries,
                    fee=fee,
                    coin_announcements=xone_announcement if len(xone_coins) > 1 else None,
...
if TYPE_CHECKING:
    from one.wallet.wallet_protocol import WalletProtocol
