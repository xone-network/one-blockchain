...
from one.consensus.blockchain import ReceiveBlockResult
...
from one.protocols.full_node_protocol import RequestProofOfWeight, RespondProofOfWeight
from one.protocols.protocol_message_types import ProtocolMessageTypes
from one.protocols.wallet_protocol import CoinState, RespondBlockHeader, RespondToCoinUpdates
from one.rpc.rpc_server import default_get_connections
from one.server.node_discovery import WalletPeers
...
from one.util.config import WALLET_PEERS_PATH_KEY_DEPRECATED, process_config_start_method
from one.util.errors import KeychainIsEmpty, KeychainIsLocked, KeychainKeyNotFound, KeychainProxyConnectionFailure
...
from one.wallet.util.new_peak_queue import NewPeakItem, NewPeakQueue, NewPeakQueueTypes
from one.wallet.util.peer_request_cache import PeerRequestCache, can_use_peer_request_cache
from one.wallet.util.wallet_sync_utils import (
...
from one.wallet.wallet_state_manager import WalletStateManager
from one.wallet.wallet_weight_proof_handler import WalletWeightProofHandler, get_wp_fork_point
...
class WalletNode:
    ...
    _server: Optional[OneServer] = None
    ...
    async def get_key_for_fingerprint(self, fingerprint: Optional[int]) -> Optional[PrivateKey]:
        try:
            ...
            self.log.warning("No keys present. Create keys with the UI, or with the 'one keys' program.")
            return None
    ...
    def initialize_wallet_peers(self):
        ...
        if self.wallet_peers is None and connect_to_unknown_peers and not testing:
            self.wallet_peers = WalletPeers(
                ...
                self.config.get("dns_servers", ["dns-introducer.onechain.vip"]),
    ...
    def on_disconnect(self, peer: WSOneConnection):
    ...
    async def on_connect(self, peer: WSOneConnection):
        if self._wallet_state_manager is None:
            return None
    ...
    async def is_peer_synced(
        self, peer: WSOneConnection, header_block: HeaderBlock, request_time: uint64
    ) -> Optional[uint64]:
        # Get last timestamp
    ...
    async def state_update_received(self, request: wallet_protocol.CoinStateUpdate, peer: WSOneConnection) -> None:
    ...
    def get_full_node_peer(self) -> Optional[WSOneConnection]:
        """
        Get a full node, preferring synced & trusted > synced & untrusted > unsynced & trusted > unsynced & untrusted
        """
        full_nodes: List[WSOneConnection] = self.get_full_node_peers_in_order()
        if len(full_nodes) == 0:
            return None
        return full_nodes[0]

    def get_full_node_peers_in_order(self) -> List[WSOneConnection]:
        """
        ...
        synced_and_trusted: List[WSOneConnection] = []
        synced: List[WSOneConnection] = []
        trusted: List[WSOneConnection] = []
        neither: List[WSOneConnection] = []
        all_nodes: List[WSOneConnection] = self.server.get_connections(NodeType.FULL_NODE)
    ...
    async def get_timestamp_for_height(self, height: uint32) -> uint64:
        ...
        peers: List[WSOneConnection] = self.get_full_node_peers_in_order()
    ...
    async def new_peak_wallet(self, new_peak: wallet_protocol.NewPeakWallet, peer: WSOneConnection):
        if self._wallet_state_manager is None:
    ...
    async def new_peak_from_trusted(self, new_peak_hb: HeaderBlock, latest_timestamp: uint64, peer: WSOneConnection):
    ...
    async def new_peak_from_untrusted(
        self, new_peak_hb: HeaderBlock, peer: WSOneConnection, request_time: uint64
    ) -> bool:
        far_behind: bool = (
    ...
    async def long_sync_from_untrusted(self, syncing: bool, new_peak_hb: HeaderBlock, peer: WSOneConnection):
        current_height: uint32 = await self.wallet_state_manager.blockchain.get_finished_sync_up_to()
    ...
    async def wallet_short_sync_backtrack(self, header_block: HeaderBlock, peer: WSOneConnection) -> int:
    ...
    async def fetch_and_validate_the_weight_proof(
        self, peer: WSOneConnection, peak: HeaderBlock
    ...
    async def validate_block_inclusion(
        self, block: HeaderBlock, peer: WSOneConnection, peer_request_cache: PeerRequestCache
    ) -> bool:
    ...
    async def fetch_puzzle_solution(self, height: uint32, coin: Coin, peer: WSOneConnection) -> CoinSpend:
    ...
    async def get_coin_state(
        self, coin_names: List[bytes32], peer: WSOneConnection, fork_height: Optional[uint32] = None
    ) -> List[CoinState]:
    ...
    async def fetch_children(
        self, coin_name: bytes32, peer: WSOneConnection, fork_height: Optional[uint32] = None
    ) -> List[CoinState]:
