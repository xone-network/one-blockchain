...
from one.seeder.crawl_store import CrawlStore
from one.seeder.peer_record import PeerRecord, PeerReliability
...
class Crawler:
    ...
    async def connect_task(self, peer):
        async def peer_action(peer: WSOneConnection):
    ...
    async def new_peak(self, request: full_node_protocol.NewPeak, peer: WSOneConnection):
        try:
            peer_info = peer.get_peer_info()
