...
from one.cmds.init_funcs import check_keys, one_full_version_str, one_init
from one.cmds.passphrase_funcs import default_passphrase, using_default_passphrase
from one.daemon.keychain_server import KeychainServer, keychain_commands
from one.daemon.windows_signal import kill
from one.plotters.plotters import get_available_plotters
from one.plotting.util import add_plot_directory
from one.server.server import ssl_context_for_root, ssl_context_for_server
from one.ssl.create_ssl import get_mozilla_ca_crt
from one.util.beta_metrics import BetaMetricsLogger
from one.util.one_logging import initialize_service_logging
from one.util.config import load_config
from one.util.errors import KeychainCurrentPassphraseIsInvalid
from one.util.json_util import dict_to_json_str
from one.util.keychain import Keychain, passphrase_requirements, supports_os_passphrase_storage
from one.util.lock import Lockfile, LockfileError
from one.util.network import WebServer
from one.util.service_groups import validate_service
from one.util.setproctitle import setproctitle
from one.util.ws_message import WsRpcMessage, create_payload, format_response
...
try:
    ...
    print("Error: Make sure to run . ./activate from the project folder before starting One.")
    quit()
...
if getattr(sys, "frozen", False):
    name_map = {
        "one": "one",
        "one_data_layer": "start_data_layer",
        "one_data_layer_http": "start_data_layer_http",
        "one_wallet": "start_wallet",
        "one_full_node": "start_full_node",
        "one_harvester": "start_harvester",
        "one_farmer": "start_farmer",
        "one_introducer": "start_introducer",
        "one_timelord": "start_timelord",
        "one_timelord_launcher": "timelord_launcher",
        "one_full_node_simulator": "start_simulator",
        "one_seeder": "start_seeder",
        "one_crawler": "start_crawler",
    }
...
class WebSocketServer:
        ...
        if self.ssl_context.minimum_version is not ssl.TLSVersion.TLSv1_3:
            self.log.warning(
                (
                    ...
                    "A future version of One will require TLS1.3."
                ),
                ssl.OPENSSL_VERSION,
            )
    ...
    def _build_plotting_command_args(self, request: Any, ignoreCount: bool, index: int) -> List[str]:
        ...
        command_args: List[str] = ["one", "plotters", plotter]

        if plotter == "bladebit":
            # plotter command must be either
            # 'one plotters bladebit ramplot' or 'one plotters bladebit diskplot'
            plot_type = request["plot_type"]
...
def launch_plotter(root_path: Path, service_name: str, service_array: List[str], id: str):
    # we need to pass on the possibly altered ONE_ROOT
    os.environ["ONE_ROOT"] = str(root_path)
...
def launch_service(root_path: Path, service_command) -> Tuple[subprocess.Popen, Path]:
    ...
    log.debug(f"Launching service {service_array} with ONE_ROOT: {os.environ['ONE_ROOT']}")
...
async def async_run_daemon(root_path: Path, wait_for_unlock: bool = False) -> int:
    # When wait_for_unlock is true, we want to skip the check_keys() call in one_init
    # since it might be necessary to wait for the GUI to unlock the keyring first.
    one_init(root_path, should_check_keys=(not wait_for_unlock))
    config = load_config(root_path, "config.yaml")
    setproctitle("one_daemon")
    ...
    try:
        with Lockfile.create(daemon_launch_lock_path(root_path), timeout=1):
            log.info(f"one-blockchain version: {one_full_version_str()}")
