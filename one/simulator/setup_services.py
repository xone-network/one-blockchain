...
from one.cmds.init_funcs import init
...
from one.introducer.introducer import Introducer
from one.protocols.shared_protocol import Capability, capabilities
from one.server.start_farmer import create_farmer_service
from one.server.start_full_node import create_full_node_service
from one.server.start_harvester import create_harvester_service
from one.server.start_introducer import create_introducer_service
from one.server.start_service import Service
from one.server.start_timelord import create_timelord_service
from one.server.start_wallet import create_wallet_service
from one.simulator.block_tools import BlockTools
from one.simulator.keyring import TempKeyring
from one.simulator.start_simulator import create_full_node_simulator_service
from one.timelord.timelord import Timelord
from one.timelord.timelord_launcher import kill_processes, spawn_process
from one.types.peer_info import PeerInfo
from one.util.bech32m import encode_puzzle_hash
from one.util.config import lock_and_load_config, save_config
from one.util.ints import uint16
from one.util.keychain import bytes_to_mnemonic
...
async def setup_wallet_node(
    ...
    with TempKeyring(populate=True) as keychain:
        ...
        service_config["xone_spam_amount"] = xone_spam_amount
...
async def setup_farmer(
    ...
    service_config["xone_target_address"] = encode_puzzle_hash(b_tools.farmer_ph, "xone")
    service_config["pool_public_keys"] = [bytes(pk).hex() for pk in b_tools.pool_pubkeys]
    service_config["port"] = port
    service_config["rpc_port"] = uint16(0)
    config_pool["xone_target_address"] = encode_puzzle_hash(b_tools.pool_ph, "xone")
