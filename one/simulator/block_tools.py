...
from one.cmds.init_funcs import create_all_ssl, create_default_one_config
...
from one.consensus.coinbase import create_farmer_coin, create_pool_coin, create_puzzlehash_for_pk, \
    create_community_coin, create_timelord_coin
from one.consensus.condition_costs import ConditionCost
from one.consensus.constants import ConsensusConstants
from one.consensus.default_constants import DEFAULT_CONSTANTS
from one.consensus.deficit import calculate_deficit
from one.consensus.full_block_to_block_record import block_to_block_record
from one.consensus.make_sub_epoch_summary import next_sub_epoch_summary
from one.consensus.pot_iterations import (
    calculate_ip_iters,
...
from one.full_node.bundle_tools import (
    best_solution_generator_from_template,
    detect_potential_template_generator,
    simple_solution_generator,
)
from one.full_node.generator import setup_generator_args
from one.full_node.mempool_check_conditions import GENERATOR_MOD
from one.full_node.signage_point import SignagePoint
from one.plotting.create_plots import PlotKeys, create_plots
from one.plotting.manager import PlotManager
from one.plotting.util import (
    PlotRefreshEvents,
    PlotRefreshResult,
...
from one.simulator.time_out_assert import time_out_assert_custom_interval
from one.simulator.wallet_tools import WalletTool
...
from one.types.blockchain_format.foliage import Foliage, FoliageBlockData, FoliageTransactionBlock, TransactionsInfo
from one.types.blockchain_format.pool_target import PoolTarget
from one.types.blockchain_format.program import INFINITE_COST
from one.types.blockchain_format.proof_of_space import ProofOfSpace
from one.types.blockchain_format.reward_chain_block import RewardChainBlockUnfinished
from one.types.blockchain_format.sized_bytes import bytes32
from one.types.blockchain_format.slots import (
...
from one.types.generator_types import BlockGenerator, CompressorArg
...
from one.util.config import config_path_for_filename, load_config, lock_config, override_config, save_config
...
from one.util.keychain import Keychain, bytes_to_mnemonic
from one.util.prev_transaction_block import get_prev_transaction_block
from one.util.vdf_prover import get_vdf_info_and_proof
...
class BlockTools:
    ...
    def __init__(
        ...
        if automated_testing:
            ...
            create_default_one_config(root_path)
            create_all_ssl(
                root_path,
    ...
    async def setup_keys(self, fingerprint: Optional[int] = None, reward_ph: Optional[bytes32] = None):
        if self.local_keychain:
            ...
            keychain_proxy = await connect_to_keychain_and_validate(
                self.root_path, self.log, user="testing-1.8.0", service="one-testing-1.8.0"
            )
        ...
        if len(self.pool_pubkeys) == 0 or len(self.farmer_pubkeys) == 0:
            raise RuntimeError("Keys not generated. Run `one keys generate`")
        ...
        try:
            if plot_keys is None:
                ...
                if pool_contract_puzzle_hash is None:
                    ...
                    pool_address = encode_puzzle_hash(pool_contract_puzzle_hash, "xone")
...
def create_test_foliage(
    ...
    Args:
        ...
        community_reward_puzzle_hash: where to pay out community reward
        timelord_fee_puzzle_hash: where to pay out timelord fee
