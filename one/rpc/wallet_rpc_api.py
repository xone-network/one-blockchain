...
from one.consensus.block_rewards import calculate_base_farmer_reward
from one.data_layer.data_layer_wallet import DataLayerWallet
from one.pools.pool_wallet import PoolWallet
from one.pools.pool_wallet_info import FARMING_TO_POOL, PoolState, PoolWalletInfo, create_pool_state
from one.protocols.protocol_message_types import ProtocolMessageTypes
from one.protocols.wallet_protocol import CoinState
from one.rpc.rpc_server import Endpoint, EndpointResult, default_get_connections
from one.server.outbound_message import NodeType, make_msg
from one.server.ws_connection import WSOneConnection
from one.simulator.simulator_protocol import FarmNewBlockProtocol
from one.types.announcement import Announcement
from one.types.blockchain_format.coin import Coin, coin_as_list
...
from one.util.errors import KeychainIsLocked
from one.util.ints import uint8, uint16, uint32, uint64
from one.util.keychain import bytes_to_mnemonic, generate_mnemonic
from one.util.path import path_from_root
from one.util.ws_message import WsRpcMessage, create_payload_dict
from one.wallet.cat_wallet.cat_constants import DEFAULT_CATS
from one.wallet.cat_wallet.cat_wallet import CATWallet
...
from one.wallet.did_wallet.did_wallet import DIDWallet
from one.wallet.did_wallet.did_wallet_puzzles import (
    DID_INNERPUZ_MOD,
    create_fullpuz,
    match_did_puzzle,
    program_to_metadata,
...
from one.wallet.nft_wallet.nft_info import NFTCoinInfo, NFTInfo
from one.wallet.nft_wallet.nft_puzzles import get_metadata_and_phs
from one.wallet.nft_wallet.nft_wallet import NFTWallet
from one.wallet.nft_wallet.uncurry_nft import UncurriedNFT
from one.wallet.notification_store import Notification
from one.wallet.outer_puzzles import AssetType
from one.wallet.puzzle_drivers import PuzzleInfo, Solver
from one.wallet.puzzles.p2_delegated_puzzle_or_hidden_puzzle import puzzle_hash_for_synthetic_public_key
from one.wallet.trade_record import TradeRecord
from one.wallet.trading.offer import Offer
from one.wallet.transaction_record import TransactionRecord
from one.wallet.uncurried_puzzle import uncurry_puzzle
from one.wallet.util.address_type import AddressType, is_valid_address
from one.wallet.util.compute_hints import compute_coin_hints
...
class WalletRpcApi:
    ...
    async def get_latest_singleton_coin_spend(
        self, peer: Optional[WSOneConnection], coin_id: bytes32, latest: bool = True
    ...
    async def _check_key_used_for_rewards(
        ...
        farmer_target = config["farmer"].get("xone_target_address")
        pool_target = config["pool"].get("xone_target_address")
    ...
    async def create_new_wallet(self, request: Dict) -> EndpointResult:
        ...
        if request["wallet_type"] == "cat_wallet":
            ...
            if request["mode"] == "new":
                ...
                from one.pools.pool_wallet_info import initial_pool_state_from_dict
    ...
    async def verify_signature(self, request) -> EndpointResult:
        ...
        # signatures made by `one keys sign`, which uses BLS_MESSAGE_AUGMENTATION_HEX_INPUT
    ...
    async def get_offer_summary(self, request) -> EndpointResult:
        ...
        from one.util.bech32m import bech32_decode, convertbits
        from one.wallet.util.puzzle_compression import decompress_object_with_puzzles
    ...
    async def check_offer_validity(self, request) -> EndpointResult:
        offer_hex: str = request["offer"]
        offer = Offer.from_bech32(offer_hex)
        peer: Optional[WSOneConnection] = self.service.get_full_node_peer()
        if peer is None:
    ...
    async def cancel_offers(self, request: Dict) -> EndpointResult:
        ...
        if cancel_all:
            ...
            asset_id = request.get("asset_id", "xone")
        ...
        key = None
        if asset_id is not None and asset_id != "xone":
    ...
    async def did_get_info(self, request) -> EndpointResult:
        ...
        return {
            ...
            "p2_address": encode_puzzle_hash(p2_puzzle.get_tree_hash(), AddressType.XONE.hrp(self.service.config)),
    ...
    async def nft_mint_bulk(self, request) -> EndpointResult:
        ...
        xone_coin_list = request.get("xone_coins", None)
        xone_coins = None
        if xone_coin_list:
            xone_coins = set([Coin.from_json_dict(xone_coin) for xone_coin in xone_coin_list])
        xone_change_target = request.get("xone_change_target", None)
        if xone_change_target is not None:
            if xone_change_target[:2] == "xone":
                xone_change_ph = decode_puzzle_hash(xone_change_target)
            else:
                xone_change_ph = bytes32(hexstr_to_bytes(xone_change_target))
        else:
            xone_change_ph = None
        ...
        if mint_from_did:
            sb = await nft_wallet.mint_from_did(
                ...
                xone_coins=xone_coins,
                xone_change_ph=xone_change_ph,
            ...
            sb = await nft_wallet.mint_from_xone(
                metadata_list,
                mint_number_start=mint_number_start,
                mint_total=mint_total,
                target_list=target_list,
