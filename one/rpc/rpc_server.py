...
from one.rpc.util import wrap_http_handler
from one.server.outbound_message import NodeType
from one.server.server import OneServer, ssl_context_for_client, ssl_context_for_server
...
from one.util.config import str2bool
from one.util.ints import uint16
from one.util.json_util import dict_to_json_str
from one.util.network import WebServer
from one.util.ws_message import WsRpcMessage, create_payload, create_payload_dict, format_response, pong
...
class RpcServiceProtocol(Protocol):
    ...
    def server(self) -> OneServer:
        ...
        # a property so as to be read only which allows OneServer to satisfy
        # Optional[OneServer]
        ...
    ...
    def get_connections(self, request_node_type: Optional[NodeType]) -> List[Dict[str, Any]]:
        ...
        one.rpc.rpc_server.default_get_connections()
        """
        ...
...
def default_get_connections(server: OneServer, request_node_type: Optional[NodeType]) -> List[Dict[str, Any]]:
