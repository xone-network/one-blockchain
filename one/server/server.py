...
from one.protocols.protocol_state_machine import message_requires_reply
from one.protocols.protocol_timing import API_EXCEPTION_BAN_SECONDS, INVALID_PROTOCOL_BAN_SECONDS
from one.protocols.shared_protocol import protocol_version
from one.server.introducer_peers import IntroducerPeers
from one.server.outbound_message import Message, NodeType
from one.server.ssl_context import private_ssl_paths, public_ssl_paths
...
from one.util.api_decorators import get_metadata
from one.util.errors import Err, ProtocolError
from one.util.ints import uint16
from one.util.network import WebServer, is_in_network, is_localhost
from one.util.ssl_check import verify_ssl_certs_and_keys
...
class OneServer:
    def __init__(
        ...
        one_ca_crt_key: Tuple[Path, Path],
        name: str = None,
    ):
        # Keeps track of all connections to and from this node.
        self.all_connections: Dict[bytes32, WSOneConnection] = {}
        ...
        one_ca_crt_path, one_ca_key_path = one_ca_crt_key
        ...
        if self._local_type in authenticated_client_types:
            ...
            self.ssl_client_context = ssl_context_for_client(
                one_ca_crt_path, one_ca_key_path, public_cert_path, public_key_path
            )
        ...
        if self._local_type in authenticated_server_types:
            ...
            public_cert_path, public_key_path = public_ssl_paths(root_path, config)
            self.ssl_context = ssl_context_for_server(
                one_ca_crt_path, one_ca_key_path, public_cert_path, public_key_path, log=self.log
            )
    ...
    async def garbage_collect_connections_task(self) -> None:
        ...
        while True:
            ...
            to_remove: List[WSOneConnection] = []
    ...
    async def start_server(self, prefer_ipv6: bool, on_connect: Callable = None):
        if self.webserver is not None:
            raise RuntimeError("OneServer already started")
    ...
    async def incoming_connection(self, request):
        ...
        if peer_id == self.node_id:
            return ws
        connection: Optional[WSOneConnection] = None
        try:
            connection = WSOneConnection(
                self._local_type,
                ws,
                self._port,
                self.log,
    ...
    async def connection_added(self, connection: WSOneConnection, on_connect=None):
    ...
    def connection_closed(self, connection: WSOneConnection, ban_time: int):
    ...
    async def incoming_api_task(self) -> None:
        ...
        while True:
            ...
            async def api_call(full_message: Message, connection: WSOneConnection, task_id):
    ...
    def get_outgoing_connections(self) -> List[WSOneConnection]:
        result = []
        for _, connection in self.all_connections.items():
    ...
    def get_full_node_outgoing_connections(self) -> List[WSOneConnection]:
        result = []
    ...
    def get_connections(self, node_type: Optional[NodeType] = None) -> List[WSOneConnection]:
        result = []
        for _, connection in self.all_connections.items():
    ...
    def is_trusted_peer(self, peer: WSOneConnection, trusted_peers: Dict) -> bool:
