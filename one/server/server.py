...
from one.protocols.protocol_timing import INVALID_PROTOCOL_BAN_SECONDS
...
from one.server.ws_connection import ConnectionCallback, WSOneConnection
...
class OneServer:
    ...
    all_connections: Dict[bytes32, WSOneConnection] = field(default_factory=dict)
    ...
    def create(
        ...
        if local_type in authenticated_server_types:
            ...
            public_cert_path, public_key_path = public_ssl_paths(root_path, config)
            ssl_context = ssl_context_for_server(
                one_ca_crt_path, one_ca_key_path, public_cert_path, public_key_path, log=log
            )
    ...
    async def incoming_connection(self, request: web.Request) -> web.StreamResponse:
        ...
        try:
            connection = WSOneConnection.create(
                self._local_type,
                ws,
                self.api,
                self._port,
    ...
    async def connection_added(
        self, connection: WSOneConnection, on_connect: Optional[ConnectionCallback] = None
    ) -> None:
    ...
    def connection_closed(self, connection: WSOneConnection, ban_time: int, closed_connection: bool = False) -> None:
    ...
    def is_trusted_peer(self, peer: WSOneConnection, trusted_peers: Dict[str, Any]) -> bool:
