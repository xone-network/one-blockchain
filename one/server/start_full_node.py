...
from one.rpc.full_node_rpc_api import FullNodeRpcApi
...
from one.util.task_timing import maybe_manage_task_instrumentation
...
def main() -> int:
    ...
    with maybe_manage_task_instrumentation(enable=os.environ.get("ONE_INSTRUMENT_NODE") is not None):
        ...
        if not service_config.get("use_one_loop_policy", True):
            target_peer_count = None
