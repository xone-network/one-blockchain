...
class Service(Generic[_T_RpcServiceProtocol]):
    def __init__(
        ...
        *,
        ...
        self._log.info(f"one-blockchain version: {one_full_version_str()}")

        self.service_config = self.config[service_name]

        self._rpc_info = rpc_info
        private_ca_crt, private_ca_key = private_ssl_ca_paths(root_path, self.config)
        one_ca_crt, one_ca_key = one_ssl_ca_paths(root_path, self.config)
    ...
    async def setup_process_global_state(self) -> None:
        ...
        proctitle_name = f"one_{self._service_name}"
    ...
    async def wait_closed(self) -> None:
        ...
        self._log.info("Waiting for OneServer to be closed")
