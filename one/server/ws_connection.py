...
from one.protocols.protocol_state_machine import message_response_ok
from one.protocols.protocol_timing import API_EXCEPTION_BAN_SECONDS, INTERNAL_PROTOCOL_ERROR_BAN_SECONDS
from one.protocols.shared_protocol import Capability, Handshake
from one.server.capabilities import known_active_capabilities
from one.server.outbound_message import Message, NodeType, make_msg
from one.server.rate_limits import RateLimiter
...
from one.util.ints import uint8, uint16
from one.util.log_exceptions import log_exceptions

# Each message is prepended with LENGTH_BYTES bytes specifying the length
from one.util.network import class_for_type, is_localhost
from one.util.streamable import Streamable

# Max size 2^(8*4) which is around 4GiB
LENGTH_BYTES: int = 4

WebSocket = Union[WebSocketResponse, ClientWebSocketResponse]
ConnectionCallback = Callable[["WSOneConnection"], Awaitable[None]]
