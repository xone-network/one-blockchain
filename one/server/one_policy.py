...
class OneSelectorEventLoop(asyncio.SelectorEventLoop):
    ...
    async def create_server(self, *args, **kwargs) -> PausableServer:  # type: ignore[no-untyped-def]
        return await _one_create_server(super(), *args, **kwargs)


if sys.platform == "win32":

    class OneProactor(asyncio.windows_events.IocpProactor):
        ...
        async def _one_accept_loop(self, listener):
            while True:
        ...
        def accept(self, listener):
            coro = self._one_accept_loop(listener)
            return asyncio.ensure_future(coro)

    class OneProactorEventLoop(asyncio.windows_events.ProactorEventLoop):
...
class OnePolicy(asyncio.DefaultEventLoopPolicy):
...
def set_one_policy(connection_limit: int) -> None:
    ...
    asyncio.set_event_loop_policy(OnePolicy())
