...
# Gets the version from the result of "one version"
...
def get_one_version() -> str:
    version: str = "0.0"
    output = subprocess.run(["one", "version"], capture_output=True)
...
if __name__ == "__main__":
    update_version(f"{os.path.dirname(__file__)}/one-blockchain-gui/package.json")
    update_version(f"{os.path.dirname(__file__)}/one-blockchain-gui/packages/gui/package.json")
